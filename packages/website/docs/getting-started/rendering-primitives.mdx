...
One of the most simplest way to visualize the primitives or even any other shapes on web is through svg image. Hence, `svgPath` and `boundingBox` properties of the primitive come in handy.
...
If you want to constantly animate it, you will need to trigger `drawInCanvas` on every frame or changes.
