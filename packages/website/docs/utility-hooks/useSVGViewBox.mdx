...
However, this hooks can help you to maintain the `viewBox` property, so you no need to hardcode it yourself.
...
| `viewBoxOptions` | `ComputeSVGViewBoxOptions` |    `{}`    | Options to construct the `viewBox`. |

1. **_(PrimitivesLike)_** `primitives`
...
2. **_(ComputeSVGViewBoxOptions)_** `viewBoxOptions`
...
You can set a `padding` so the the path looked nice in the center.

If multiple primitives are given, `useSVGViewBox` will merged all them in the plane, and compute one `viewBox` to accomodate all of them.
...
But that would required all four of them are given. If one of them is incomplete, they all be ignored.
